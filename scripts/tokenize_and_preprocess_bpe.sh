...
test_bpe_src=$datadir/bpe/test.$src
...
echo `date '+%Y-%m-%d %H:%M:%S'` "Apply BPE to test data" 
subword-nmt apply-bpe -c $bpe_vocab --input $tokdir/${test_tok}.$src.atok --output $test_bpe_src 
